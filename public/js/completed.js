...
const checkNumOfTasks = async() => {
    ...
    nextPage.addEventListener("click", async function() {
        ...
        if (pageNumber != 1) {
            ...
            return await getTasks(`/tasks?completed=true&limit=10&skip=${(pageNumber -1) * 10}`)
        } else {
            return await getTasks(`/tasks?completed=true&limit=10`);
        }
    ...
    return {url:`/tasks?completed=true&limit=10`,task_length: listLength}
